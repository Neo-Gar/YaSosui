...
const SUI_ESCROW_FACTORY_ADDRESS =
    process.env.SUI_ESCROW_FACTORY_ADDRESS ||
    "0xfab87636c79339242adf52df5f5ef84f31191be7366ff0a83c8b030236a2b931";
...
describe("Ethereum to Sui Cross-Chain Swap", () => {
    ...
    beforeAll(async () => {
        ...
        console.log(
            "[SUI] Resolver address:",
            dstChainResolver.getPublicKey().toSuiAddress(),
        );
    ...
    describe("Fill", () => {
        it("should swap Ethereum USDC -> Sui USDC. Single fill only", async () => {
            ...
            txbEscrow.moveCall({
                ...
                arguments: [
                    txbEscrow.object(
                        "0x3f3f2cfd1a303d474e92322e38027700f1b86c298b4a778f536368e8faef940e",
                    ), // factory object
                    txbEscrow.pure("vector<u8>", hexToBytes(orderHash)), // order_hash
                    txbEscrow.pure(
                        "vector<u8>",
                        hexToBytes(dstImmutables.hashLock.toString()),
                    ), // hash_lock
                    ...
                    txbEscrow.pure(
                        "vector<u8>",
                        hexToBytes(dstImmutables.timeLocks.toString()),
                    ), // time_locks
            ...
            console.log(
                `[${dstChainId}]`,
                `Withdrawing funds for user from ${escrow_id}`,
            );
    ...
    describe("Cancel", () => {
        it("should cancel swap Ethereum USDC -> Sui USDC", async () => {
            ...
            txb.transferObjects(
                [coin],
                txb.pure("address", dstChainResolver.getPublicKey().toSuiAddress()),
            );
            ...
            console.log(
                `[${dstChainId}]`,
                `Created dst deposit for order ${orderHash} in tx ${signature_sui}`,
            );
            ...
            const [cancelCoin] = cancelTxb.splitCoins(cancelTxb.gas, [
                cancelTxb.pure("u64", 250000n),
            ]);
